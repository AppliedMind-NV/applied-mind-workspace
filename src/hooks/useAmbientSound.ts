...
/**
 * LO-FI — Mellow jazzy chords with vinyl crackle and tape wobble.
 * Uses detuned oscillators for warm chord pads, filtered noise for vinyl texture.
 */
function buildLoFi(ctx: AudioContext, masterGain: GainNode) {
  const nodes: (AudioBufferSourceNode | OscillatorNode)[] = [];

  // Chord pad — stack of detuned sine/triangle oscillators (Cmaj7-ish voicing)
  const chordNotes = [261.6, 329.6, 392.0, 493.9]; // C4, E4, G4, B4
  const padGain = ctx.createGain();
  padGain.gain.value = 0.08;
  const padLP = ctx.createBiquadFilter();
  padLP.type = "lowpass";
  padLP.frequency.value = 800; // Muffled, warm
  padLP.Q.value = 0.7;
  ...
  // Slow chord movement — LFO modulates the filter cutoff
  const chordLfo = ctx.createOscillator();
  chordLfo.type = "sine";
  chordLfo.frequency.value = 0.05; // Very slow breathing
  const chordLfoGain = ctx.createGain();
  chordLfoGain.gain.value = 200;
  chordLfo.connect(chordLfoGain);
  chordLfoGain.connect(padLP.frequency);
  chordLfo.start();
  nodes.push(chordLfo);

  // Vinyl crackle — heavily filtered, quiet noise
  ...
  for (let i = 0; i < crackleData.length; i++) {
    ...
    crackleData[i] = Math.random() < 0.002 ? (Math.random() * 2 - 1) * 0.8 : (Math.random() * 2 - 1) * 0.02;
  }
  ...
  // Tape wobble — subtle pitch modulation on the pad
  const wobble = ctx.createOscillator();
  wobble.type = "sine";
  wobble.frequency.value = 0.4; // Gentle tape warble
  const wobbleGain = ctx.createGain();
  wobbleGain.gain.value = 3; // Subtle detune in cents
  wobble.connect(wobbleGain);
  chordNotes.forEach((_, i) => {
    wobbleGain.connect((nodes[i] as OscillatorNode).detune);
  });
  wobble.start();
  nodes.push(wobble);

  // Sub bass — very low sine for warmth
  ...
  return {
    sources: nodes,
    start: () => {}, // All oscillators already started
  };
}
